document.addEventListener('DOMContentLoaded', () => {
    ...
    let isFileConverted = false; // State to track if the current file is converted
    ...
    const checkLicenseAndToggleUI = () => {
        dropZone.classList.toggle('disabled', !isLicenseValid || isFileConverted);
    ...
    const updateFileList = () => {
        ...
        uploadedFiles.forEach((file, index) => {
            ...
            let fileInfoHTML = `<span>${file.name} (${fileSize} MB)</span>`;
            if (isFileConverted) {
                listItem.classList.add('converted');
                fileInfoHTML += `<span class="checkmark">✔</span>`;
            }
            
            listItem.innerHTML = fileInfoHTML;
    ...
    const removeFile = (index) => {
        ...
        isFileConverted = false; // Reset the converted state
        updateFileList();
        resetStatusUI(); // Hide status messages
        convertButton.parentElement.parentElement.style.display = 'flex'; // Show the convert button again
        checkLicenseAndToggleUI(); // Re-enable dropzone and check button states
    };
        ...
        xhr.onload = async () => {
            try {
                ...
                if (xhr.status >= 200 && xhr.status < 300) {
                    isFileConverted = true; // Set the converted flag
                    progressBar.style.display = 'none';
                    convertButton.parentElement.parentElement.style.display = 'none';
                    
                    // Create clickable download link and instructional text
                    statusMessage.innerHTML = `<a href="${result.downloadUrl}" download>Conversion Successful! Click Here to Download.</a>
                                               <p id="next-step-notice">To convert another file, please remove the one above first.</p>`;
                    
                    sessionHistory.unshift({ sourceFiles: originalFilesForHistory.map(f => f.name) });
                    updateHistoryList();
                    updateFileList(); // Redraw the file list to show the converted state
                    ...
                    if (!licenseKeyInput.disabled) { checkLicenseAndToggleUI(); }
                }
            } catch (e) {
